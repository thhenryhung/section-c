...
export async function putJSONFile(
  ...
  const res = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/contents/${path}`, {
    ...
    body: JSON.stringify({
      ...
      content: encoded,
      sha,
      committer: { name: 'Section C Admin', email: 'noreply@users.noreply.github.com' },
    }),
  })
...
export async function triggerWorkflow(
  ...
  // Opportunistic: a rebuild is a durability nicety here, not something the
