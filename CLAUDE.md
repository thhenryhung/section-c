...
real Section C surname, and a real classmate's email local part used as an example
...
Everything real lives in a separate **private** repo, `section-c-data`, cloned as a
sibling directory (`../section-c-data`). CI reads it with a read-only token.
...
- **Admin panel** (`functions/api/admin/*`, `src/pages/AdminPage.tsx`): a
  ...
  their own narrowly-scoped write path into the *private* `section-c-data`
...
**Brand:** Section C's own green (`--color-green-*` in `src/index.css`) carries all
...
| `npm run data:parse` | Real class cards → `roster.json`. Needs `../section-c-data`. |
...
- **`section-c-data/photos/` must hold the actual image files, not just the
...
- **Deferred until after the MVP**, by decision, not oversight: LinkedIn and
  Instagram (a sheet covering 40 of 90 exists at `section-c-data/socials.csv`),
