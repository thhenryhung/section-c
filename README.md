...
A section website for **HBS MBA 2028, Section C** — a directory you can actually
...
**Live at** [section-c.pages.dev](https://section-c.pages.dev) — passphrase required,
shared with the section separately from this repo.

Unlocking lands on a welcome page (`src/pages/WelcomePage.tsx`) with one card per
tab below, not straight into the directory.

## The four tabs

| Tab | What it does |
|---|---|
| **Jirectory** | Everyone's photo, contact details, home region and pre-MBA background. Fuzzy search across all of it, dropdown filters per facet, a card/table view toggle (card by default), and a "people like them" panel that surfaces classmates with overlapping interests. |
| **Jalendar** | Section events, socials and birthdays in a month grid or a flat timetable, with an `.ics` export you can subscribe to. Course times are deliberately not shown — see [Non-obvious things](CLAUDE.md#non-obvious-things). |
| **Jocial** | Dinners of six and coffee chats, assigned automatically every three weeks, alternating between the two. Allocation minimises repeat encounters. Shows your group, how many of the section you have met, and how to sit a round out. Rounds are generated by an organiser running `npm run pair` — there is no in-app "generate" button, by design. |
| **Just for Fun** | Photo-to-name and name-to-photo drills with spaced repetition, so it keeps testing the faces you actually get wrong. |
...
- **A separate private repo** (`section-c-data`) holds the real roster, photos and
...
| `npm run pair -- --kind dinner --date 2026-10-08` | Generate a meetup round. Add `--dry-run` to preview. |
