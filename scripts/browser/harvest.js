/**
 * Class-card harvester — run LAST, on the Section C search results page.
 *
 ...
 *
 * HOW TO RUN
 *   1. Log in, open the Section C advanced search results (all 90 showing).
 ...
 *
 * Then move that file to  section-c-data/raw/classcards.json  and run:
 *   npm run data:parse
 *
...
(async () => {
   ...
   * of a permanently lost character. Found via "Banco de Cr�dito del Per�".
   */
  ...
  if (rows.length === 0) {
    console.error('No result rows found. Are you on the Section C search results page?')
    return 'aborted'
  }
  ...
  console.log('Next: move it to section-c-data/raw/classcards.json, then `npm run data:parse`.')
