...
|---|---|---|
| `thhenryhung/section-c` | **Public** | This code. Anyone can fork and PR. |
| `thhenryhung/section-c-data` | **Private** | The real roster, photos, meetup history. |

Create both empty on GitHub, then push this code to the public one:

```bash
git remote add origin https://github.com/thhenryhung/section-c.git
...
| `DATA_REPO_TOKEN` | Fine-grained PAT with **read-only Contents** on `section-c-data`, and nothing else |
...
| Name | Value |
|---|---|
| `DATA_REPO` | `thhenryhung/section-c-data` |
...
#   Excel → Save As → CSV UTF-8 → section-c-data/classcards-export.csv

# Harvest the class cards — see scripts/browser/README.md
#   produces section-c-data/raw/ and section-c-data/photos/
